...
import falconserver.slot.machine_candy
...
doTests:
    ...
    template spinCandy(bet: int64, stage: Stage, field:seq[int8]):seq[SpinResult] =
        ...
        newSlotMachineCandy(slotMachineDesc("falconserver/resources/slot_004_candy.zsm")).getFullSpinResult(nil, bet div lines, lines, stage, field, "")
    ...
    template getTotalPayout(spinRes:seq[SpinResult], bet:int64):int64 =
        newSlotMachineCandy("falconserver/resources/slot_004_candy.zsm").getFinalPayout(bet, spinRes)
    ...
    test:
        ...
        checkCond(line1Payout.payout == 150, "Incorrect payout for wild")
        checkCond(getTotalPayout(spinRes, betPerLine) == 150 * lines * betPerLine - bet, "Incorrect math")
